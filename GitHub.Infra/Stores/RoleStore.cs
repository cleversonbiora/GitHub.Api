...
namespace GitHub.Infra.Stores
{
    public class RoleStore : IRoleStore<ApiRole>
    {
        public async Task<IdentityResult> CreateAsync(ApiRole role, CancellationToken cancellationToken)
        {
            ...
            using (var connection = ConnectionFactory.GetGitHubConnection())
            {
...
//CREATE INDEX[IX_ApplicationUser_NormalizedUserName] ON[dbo].[ApplicationUser]
//([NormalizedUserName])
